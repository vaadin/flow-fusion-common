...
import { connectionState, ConnectionState, type ConnectionStateChangeListener } from './ConnectionState.js';

declare global {
  interface VaadinFlow {
    clients: Readonly<Record<string, VaadinFlowClient>>;
  ...
  interface VaadinFlowClient {
    ...
    isActive(): boolean;
    sendEventMessage?(id: number, eventName: string, data: unknown): void;
  }
...
interface HTMLRouterContainer extends HTMLElement {
  onBeforeEnter?(ctx: NavigationParameters, cmd: PreventAndRedirectCommands, router: Router): void | Promise<unknown>;
  onBeforeLeave?(ctx: NavigationParameters, cmd: PreventCommands, router: Router): void | Promise<unknown>;
...
interface FlowRoot extends HTMLElement {
...
const clients: Record<string, VaadinFlowClient> = {};
const references: Record<string, HTMLElement> = {};

// flow uses body for keeping references
const flowRoot: FlowRoot = window.document.body as FlowRoot;
flowRoot.$ = references;

const ROOT_NODE_ID = 1; // See StateTree.java

function getClients(): readonly VaadinFlowClient[] {
...
export class Flow {
  ...
  readonly #baseRegex = /^\//u;
  readonly #appShellTitle: string;
   ...
   *
   * The `FlowRoute` object `path` property handles unknown route,
    ...
    document.addEventListener(
      ...
      (event) => {
        if (event.target instanceof HTMLElement) {
          if (event.target.hasAttribute('router-link')) {
            this.#navigation = 'link';
          } else if (event.composedPath().some((node) => node instanceof HTMLElement && node.nodeName === 'A')) {
  ...
  get #action(): (params: NavigationParameters) => Promise<HTMLRouterContainer> {
    ...
    return async (params: NavigationParameters) => {
      ...
      if (connectionState.online) {
        try {
          ...
          if (error instanceof FlowUiInitializationError) {
            ...
            connectionState.state = ConnectionState.CONNECTION_LOST;
      ...
      this.container.onBeforeEnter = async (ctx, cmd) => this.#flowNavigate(ctx, cmd);
      // For covering the 'server -> client' use case
      this.container.onBeforeLeave = async (ctx, cmd) => this.#flowLeave(ctx, cmd);
  ...
  async #flowLeave(ctx: NavigationParameters, cmd?: PreventCommands): Promise<unknown> {
  ...
  async #flowNavigate(ctx: NavigationParameters, cmd?: PreventAndRedirectCommands): Promise<HTMLElement> {
    if (this.response) {
      return new Promise((resolve) => {
        this.loadingStarted();
        ...
        this.container.serverConnected = (cancel, redirectContext?: NavigationParameters) => {
          if (cmd && cancel) {
            resolve(cmd.prevent());
          } else if (cmd?.redirect && redirectContext) {
  ...
  private getFlowRouteQuery(context: NavigationParameters | Location): string {
    return context.search?.substring(1) ?? '';
  }

  // import flow client modules and initialize UI in server side.
  async #flowInit(): Promise<AppInitResponse> {
    // Do not start flow twice
    if (!this.isFlowClientLoaded()) {
      Vaadin.Flow.nonce = this.#findNonce();
      ...
      references[appId] = this.container;
  ...
  #findNonce(): string | undefined {
    return (
      Array.from(document.head.getElementsByTagName('script'))
        .find((tag) => tag.hasAttribute('nonce'))
  ...
  // in the browser.
  private async flowInitClient(clientMod: unknown): Promise<void> {
    clientMod.init();
  ...
  private async flowInitUi(): Promise<AppInitResponse> {
    ...
    const initial = $wnd.Vaadin.TypeScript?.initial;
    if (initial) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const httpRequest = xhr as unknown;
